...
def execute():
    ...
    cursor.execute('''
    CREATE VIEW v_satisfacao_geral AS
    SELECT DISTINCT
        employer_name,
    ...
    cursor.execute('''
    CREATE VIEW v_reclamacoes_clientes AS
    SELECT DISTINCT
        Instituicao_financeira,
        Ano,
        Trimestre,
        Quantidade_total_de_reclamacoes,
        CAST(REPLACE(Quantidade_total_de_clientes_CCS_e_SCR, ',', '') AS UNSIGNED) AS total_clientes,
        Quantidade_total_de_reclamacoes * 1.0 / CAST(REPLACE(Quantidade_total_de_clientes_CCS_e_SCR, ',', '') AS UNSIGNED) AS reclamacoes_por_cliente
    FROM
        dados_finais
    WHERE
    ...
    cursor.execute('''
    CREATE VIEW v_crescimento_empresa AS
    SELECT DISTINCT
        employer_name,
        Ano,
        Trimestre,
    ...
    cursor.execute('''
    CREATE VIEW v_comparacao_segmentos AS
    SELECT DISTINCT
        Segmento_x,
        AVG(Geral) AS media_geral,
        AVG(Cultura_e_valores) AS media_cultura_valores,
    ...
    cursor.execute('''
    CREATE VIEW v_indicadores_financeiros AS
    SELECT DISTINCT
        Instituicao_financeira,
        Ano,
        Trimestre,
